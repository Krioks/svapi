...
def GetAcsessToken(app_id):
    if os.listdir().count('acsess_token.dat'):
        r = open('acsess_token.dat')
        acsess_token = r.read()
        r.close()
        kernel32 = ctypes.windll.kernel32
        attr = kernel32.GetFileAttributesW('acsess_token.dat')
        kernel32.SetFileAttributesW('acsess_token.dat', attr | 2)
...
def main():
	app_id = 6231475
	acsess_token = GetAcsessToken(app_id)
	session = vk.Session(acsess_token=acsess_token)
